...
相比synchronized，ReenTrantLock增加了一些高级功能。主要来说主要有三点：**①等待可中断；②可实现公平锁；③可实现选择性通知（锁可以绑定多个条件）；4、超时获取锁 5、ReenTrantLock需要手动释放锁 6、尝试非阻塞的获取锁**

- **ReenTrantLock提供了一种能够中断等待锁的线程的机制**，通过lock.lockInterruptibly()来实现这个机制。也就是说正在等待的线程可以选择放弃等待锁，改为处理其他事情。
...
- **ReentrantLock.tryLock()尝试非阻塞的获取锁
...
synchronized(this)：当一个线程访问ObjectService的一个synchronized (this)同步代码块时，其它线程对同一个ObjectService中其它的synchronized (this)同步代码块的访问将是堵塞，这说明synchronized (this)使用的对象监视器是一个。

synchronized(class)：静态同步synchronized方法与synchronized(*.class)代码块一样，Class锁对类的所有对象实例起作用

synchronized(任意自定义对象)：使用synchronized(任意自定义对象)进行同步操作，对象监视器必须是同一个对象。不过不是同一个，运行就是异步执行了。
