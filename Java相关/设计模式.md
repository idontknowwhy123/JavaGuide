...
抽象产品角色：运动员
具体产品角色：篮球运动员，足球运动员（实现运动员接口）
工厂角色：体育协会（获得篮球和足球运动员对象）
...
具体产品角色：篮球运动员，足球运动员（实现运动员接口）
抽象工厂角色：体育协会
具体工厂角色：篮球协会（篮球运动员），足球协会（足球运动员）（实现体育协会的接口）
好处：如果需要再加入产品类（比如加多个“乒乓球运动员”）的话就不再需要修改工厂类了，而只需相应的再添加一个实现了抽象工厂接口（“体育协会”接口）的具体工厂类
...
4.具体产品角色：这是实现抽象产品接口的具体产品类。
抽象产品角色：餐具和食物
具体产品角色：筷子和面包（分别实现餐具和食物的接口）
抽象工厂角色：生产餐具和相应食物的工厂
具体工厂角色：生产筷子和面包的工厂（实现抽象工厂的接口）
1.单例模式
1.1 定义
